        ...
        <Row>
          <Col span="5">
            <Card style="margin-bottom: 8px;">
              <div v-model="gold_profile">
                <div style="overflow: hidden;margin-bottom: 12px"  @click="getUser">
                  <div style="float: left;margin-top: 20px">头像：</div>
                  <img :src="gold_profile.avatar" alt="" width="60px" height="60px" style="border-radius: 50%;float: left">
                </div>
                <div class="user_data">用户ID：{{gold_profile.user_id}}<div class="z_show" @click="buttomID">{{IDText}}</div></div>
                <div class="user_data">名字：{{gold_profile.name}}<div class="z_show" @click="buttomName">{{NameText}}</div></div>
                <div class="user_data">电话：{{gold_profile.mobile}}<div class="z_show" @click="buttomMobile">{{MobileText}}</div></div>
                <div class="user_data">年龄：{{gold_profile.age == null ? '' : gold_profile.age + '岁'}}</div>
                <div class="user_data">性别：{{gold_profile.sex}}</div>
                <div class="user_data">身高：{{gold_profile.stature == null ? '' : gold_profile.stature + 'cm'}}</div>
                <div class="user_data">学历：{{gold_profile.degree}}</div>
                <div class="user_data">收入：{{gold_profile.income}}</div>
                <div class="user_data">信仰：{{gold_profile.belief}}</div>
                <div class="user_data">婚姻状态：{{gold_profile.state}}</div>
                <div class="user_data">工作城市：{{gold_profile.province}} {{gold_profile.city}}</div>
                <div class="user_data">出生城市：{{gold_profile.resident_province}} {{gold_profile.resident_city}}</div>
                <div class="user_data">加入时间：{{gold_profile.created_at}}</div>
              </div>
            </Card>
          </Col>
          <Col span="5"  style="margin-left: 15px">
            <Card style="margin-bottom: 8px;">
              <div style="font-weight: bold;font-size: 20px;text-align: center;margin-bottom: 24px">择偶需求</div>
              <div v-model="gold_profile">
                <div class="demand">性格：{{gold_profile.lover_character}}</div>
                <div class="demand">身高：{{gold_profile.lover_stature}}</div>
                <div class="demand">学历：{{gold_profile.lover_degree}}</div>
                <div class="demand">收入：{{gold_profile.lover_income}}</div>
                <div class="demand">最大年龄：{{gold_profile.lover_max_birthday}}</div>
                <div class="demand">最小年龄：{{gold_profile.lover_min_birthday}}</div>
                <div class="demand">工作出差：{{gold_profile.lover_post_out}}</div>
                <div class="demand">考虑因素：{{gold_profile.lover_standrad}}</div>
                <div class="demand">婚姻状态：{{gold_profile.lover_state}}</div>
                <div class="demand">综合条件：{{gold_profile.lover_condition}}</div>
                <div class="demand">是否接受孩子：{{gold_profile.lover_has_child}}</div>
              </div>
            </Card>
          </Col>
          <Col span="6" style="margin-left: 15px">
            <Card style="margin-bottom: 8px">
              <div class="choice">
                <span class="titleStyle">信仰：</span>
              ...
              <div class="choice">
                ...
                <i-select v-model="child_value" style="width:100px;" @on-change="getlist(1)">
...
export default {
  ...
  methods: {
    buttomID () {
      ...
      if (this.showID) {
        this.newID = this.gold_profile.user_id
        this.gold_profile.user_id = this.gold_profile.user_id.toString().slice(0, 0) + '*****'
        this.IDText = '展示'
      } else {
        return this.gold_profile.user_id = this.newID, this.IDText = '隐藏'
      }
    },
    buttomName () {
      this.showName = !this.showName
      if (this.showName) {
        this.newName = this.gold_profile.name
        this.gold_profile.name = this.gold_profile.name.slice(0, 1) + '**'
        this.NameText = '展示'
      } else {
        return this.gold_profile.name = this.newName, this.NameText = '隐藏'
      }
    },
    buttomMobile () {
      this.showMobile = !this.showMobile
      if (this.showMobile) {
        this.newMobile = this.gold_profile.mobile
        this.gold_profile.mobile = this.gold_profile.mobile.substr(0, 3) + '********'
        this.MobileText = '展示'
      } else {
        return this.gold_profile.mobile = this.newMobile, this.MobileText = '隐藏'
      }
    },
    getlist (page) {
      let self = this,
      ...
      if (this.activeTab == 'screen') {
        ...
        uAxios.get(`admin/other/single/profiles/` + this.id + `?page=${page}&status=${self.activeTab}&keyword=${encodeURIComponent(self.searchKeyword)}${transmit}`)
          .then(res => {
            ...
            self.gold_profile = result.gold_profile
