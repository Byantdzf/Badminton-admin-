<template>
  <div class="hello">
    <Card style="margin-top: 18px;">
      ...
      <Row>
        <Col span="11" style="min-height: 60vh">
        ...
        <Col span="11" offset="1" style="min-height: 60vh">
          <Card title="活动筛选" icon="android-favorite" style="margin-bottom: 36px">
            <span class="_bold"> 搜索活动：</span>
            <Select v-model="activity_id" style="width: 300px;" filterable @on-change="addMember" clearable>
              <Option v-for="item in activity_list" :value="item.id" :key="item.id">{{item.theme}}</Option>
            </Select>
          </Card>
          <Card title="指定省市" icon="android-favorite" style="margin-bottom: 40px">
            <span class="_bold"> 搜索城市：</span>
            <Select v-model="city_id" style="width: 300px;" filterable @on-query-change="cityList" clearable>
            ...
            <div class="bc_box">
              <div class="font_16 bc_title">
                ...
                <div style="border-bottom: 1px solid #e9e9e9;padding-bottom:6px;margin: 6px 0;color: #ff0517;"
                  ...
                  <div v-else-if="information.length>0">
                    <span v-for="item,index in this.information">{{item.name}}、</span>
                  </div>
                  <div v-else-if="activity_list.length>0">
                    <span v-for="item,index in this.member_name">{{item}}、</span>
                  </div>
                  <div v-if="city_list">
                    <span v-for="item,index in this.city_list">{{item.name}}</span>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    activityList () {
      let self = this
      uAxios.get(`admin/activities?nopage=1`)
        .then(res => {
          let result = res.data.data
          self.activity_list = result.map((item) => {
            return {
              id: item.id,
              theme: item.theme
            }
    ...
    activityMember () {
      let self = this
      if (self.activity_id === undefined) {
        self.member_name = []
        self.activity_member = []
      } else {
        uAxios.get(`admin/activity/` + self.activity_id + `/members`)
          .then(res => {
            let result = res.data.data
            for (let i of result.data) {
              self.activity_member.push(i.user_id)
              self.member_name.push(i.name)
            }
          })
      }
    },
    cityList (value) {
      let self = this
      uAxios.get(`admin/city/addresses?keyword=${value}`)
        .then(res => {
          let result = res.data.data
          console.log(result.data)
          if (value) {
            self.city_list = result.data.map((item) => {
              return {
                id: item.id,
                name: item.name
              }
    ...
    sendMessages () { // 发送
      ...
      } else if (this.activity_list.length > 0) {
        data = {content: this.content, user_ids: this.activity_member}
      } else {
        data = {content: this.content, ...this.sendTypes}
      }
      console.log(data, '666')
      if (this.city_list.length > 0) {
        uAxios.post(`admin/send/assistant/message?type=CITY&city=` + this.city_name, data)
          .then(res => {
            let {code} = res.data
            if (code === 0) {
