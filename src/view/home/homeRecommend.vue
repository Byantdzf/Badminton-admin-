      ...
      <Tabs @on-click="getTab">
        <TabPane label="首页推荐" name="0">
          <div style="margin-bottom: 22px">
            ...
            <span>
              处理查询：
              <i-select v-model="tabName" style="width:120px;" placeholder="选择类型..." clearable>
                <i-option v-for="item,index in tab" :value="item.jump" :key="index">{{ item.title }}</i-option>
              </i-select>
            </span>
            <span @click="getlist">
...
export default {
  ...
  data() {
    return {
      ...
      id: '',
      tabName: '2',
      tab: [{title: '全部', jump: '2'}, {title: '未处理', jump: '0'}, {title: '已处理', jump: '1'}],
      modal1: false,
      text: '确认',
      content: '',
      orgColumns: [
  ...
  methods: {
    ...
    getlist (page) {
      let self = this
      self.loading = true
      uAxios.get(`admin/home/recommends/v2?page=${page}&status=${self.tabName}&keyword=${self.searchKeyword}`)
        .then(res => {
